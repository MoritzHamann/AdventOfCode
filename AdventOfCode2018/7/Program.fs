...
type Node = {name: string; pre: string list; post: string list; finished: bool} with
    member this.addReqs reqs = {this with pre=List.append this.pre reqs}
    member this.removeReq req = {this with pre=List.filter (fun i -> i <> req) this.pre}
    member this.removeReqs reqs = {this with pre=List.filter (fun i -> not(List.contains i reqs)) this.pre}
    member this.addPosts posts = {this with post=List.append this.post posts}
...
[<EntryPoint>]
let main argv =
    let depenencies = System.IO.File.ReadLines argv.[0] |> Seq.map Instruction.Parse |> Seq.toList
